...
# Runtime stage: what ships and what Trivy scans. The stage NAME is
# load-bearing: ci.yml passes `no-cache-filters: runtime` to the build, so
# the apt-get upgrade below is re-executed on every CI build instead of
# being restored from the layer cache.
