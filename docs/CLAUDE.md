...
1. **Compliance Invariants are code, not guidelines.** INV-1 through INV-6 (Section 4 of the spec) are tested in `tests/test_invariants.py`. If a requested behavior would violate any invariant, stop and record it in `DECISIONS.md` instead of implementing.
...
5. **Provider interfaces are sacred.** `EmbeddingProvider` and `LLMProvider` (in `process/embedder.py` and `generate/llm.py`) gate every model call. Never hard-code a model name in business logic.
...
- LLM provider: pluggable, default `openai` with model from `LLM_MODEL`. The `echo` provider is for tests.
