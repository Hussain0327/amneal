...
- LLM provider: pluggable, default `openai/gpt-4o-mini`. Cheap, supports JSON-only response format which the extractor needs. The production model and the on-prem question are the IT team's call. (2026-08-11 follow-up: both are settled. The code default is now `gpt-5.4-nano` in `config/settings.py`, and production runs `LLM_PROVIDER=databricks` against an open-weight model hosted inside the company's own Databricks tenant. See the 2026-08-11 entries at the bottom.)
...
- SQLite via SQLModel at `data/regwatch.db`. Per spec section 7. (removed in R5: Postgres is now the only structured store.)
- Refusal text is a verbatim constant in `Settings.refusal_text`. Spec section 11 nails the exact string; keeping it in one place makes it testable and uniform.
...
- CI thresholds match spec section 12: `recall@8 >= 0.90`, `citation_precision >= 0.95`, `refusal_accuracy >= 0.95`. `faithfulness` is reported, not gated - sentence-level citation is too strict for prose.
...
- **Orange Book `type` is not the combination-product Type 1-9.** OB's `type` column is marketing status (RX/OTC), not the 21 CFR 3.2(e) combination type. The Combination Product cell is `manual`: dosage form / delivery system from Drugs@FDA are surfaced as evidence; the determination is the analyst's (INV-3). Same ruling for Patents/Priority/First-to-Market/eFTF/Labeling Carveouts - OB supplies the raw patent/exclusivity rows as evidence, never the paragraph classification or eligibility.
...
> record; see `docs/ARCHITECTURE.md` section 9 for current behavior.
>
> **(2026-08-11 follow-up):** the managed Postgres is no longer Supabase. Prod
> runs on Databricks Lakebase. Prod embeddings are no longer OpenAI
> `text-embedding-3-small` either. Both changes are recorded at the bottom of
> this file.
...
- **`Qwen3EmbeddingProvider` + `DatabricksProvider` shipped fully dormant:**
  ...
  model preference.
- **2026-08-11 follow-up: nothing here is dormant any more.** Generation
  flipped to Databricks on 2026-07-28 and the Qwen3 embedding profile was
  promoted on 2026-07-30. Both entries are at the bottom of this file.
...
- **2026-08-11 follow-up, three parts of this entry are out of date.** The
  served model is `gpt-oss-120b-080525`, not gpt-oss-20b: the alias was
  repointed on 2026-08-05. "Supabase stays the datastore" was reversed and prod
  now runs on Lakebase. OpenAI no longer serves embeddings either, so the
  "one remaining exfil point" is closed. All three are recorded at the bottom
  of this file.
...
- **Shipped LIVE but UNARMED** (auto-deployed Jul 29 14:41 UTC;
  ...
  verdict.
- **2026-08-11 follow-up: the precondition is met, the arming state is not
  recorded here.** The embedding flip landed 2026-07-30, so the both-halves
  boot rule can pass now. Whether `D1_ENFORCED` is actually set on Fly today
  was not checked, so treat that as unverified. The allowlist and the runtime
  served-model check ship either way, and the allowlist must carry both the
  alias and the served id.
...
- **Not wired into the app yet.** Next: a 1024-dim embedding profile, a
  ...
  arming `D1_ENFORCED`.
- **2026-08-11 follow-up: it is wired and serving.** The profile was created
  and the corpus re-embedded on 2026-07-30. See the entry at the bottom.
...
- **Owner amendment to the "no regulatory recommender" rule (docs/CLAUDE.md
  ...
  in a refusal. (2026-08-11 follow-up: v7 renamed the headline rule to "cite
  the facts, talk like a person", but the part that matters here is unchanged.
  A regulatory claim is a source fact and an uncited source fact is still
  dropped.) Scope note: the vendored pipeline currently emits NO
...
- **An empty flag secret reads as OFF.** `REGWATCH_PROSE_SYNTHESIS=""` falls
  ...
  outage.
- **2026-08-11 follow-up: refuse-or-cite is no longer the policy.** v7
  replaced it on 2026-08-10 and is on in prod. The v6 format flag is on in
  prod too. See the bottom of this file.
...
- **2026-08-11 follow-up: it is no longer dark.** `REGWATCH_SELECTIVE_CITATION`
  is on in prod, along with `REGWATCH_PROSE_SYNTHESIS` and
  `REGWATCH_LIVE_DRAFT`. See the last entry in this file.
...
## Prod embeddings moved in-tenant to Qwen3 (Jul 30 2026, recorded Aug 11 2026)

- **The flip happened.** `workspace.default.regwatch-embed` now serves every
  query embedding and every corpus embedding. Active profile
  `ep_2e7368b354d911ea3a013c3125e276c2`, created 2026-07-30 15:57 UTC, 1024
  dimensions, provider recorded in the database as `qwen3`.
- **Coverage is complete.** 5,494 of 5,494 chunks are embedded on that profile,
  checked against the live database on 2026-08-11. It is the only row in
  `embedding_profile`, so there is no second live vector space.
- **How the switch actually reads.** Retrieval picks its arm from
  `ACTIVE_EMBEDDING_PROFILE`. Only the `legacy` arm ever looks at
  `EMBEDDING_PROVIDER`. Prod runs a real profile, so the
  `EMBEDDING_PROVIDER = "openai"` line still sitting in `fly.toml` no longer
  affects the query path. Do not read that line as the live setting.
- **OpenAI is the rollback path, not current state.** `OPENAI_API_KEY` is still
  set on Fly and the OpenAI provider still ships and is still tested. It serves
  nothing in production today.
- **The 0.30 refusal threshold did not move with the vector space.** That is why
  the threshold entry further up is now stale in a second way.

## The serving alias was repointed to gpt-oss-120b (Aug 5 2026, recorded Aug 11 2026)

- **Same alias, bigger model.** `workspace.default.regwatch` used to route to
  `system.ai.gpt-oss-20b`. On 2026-08-05 it was repointed to
  `system.ai.gpt-oss-120b`. The served id today is `gpt-oss-120b-080525`.
  Anything still saying gpt-oss-20b is out of date.
- **No deploy was needed, which is exactly why the runtime guard exists.** A
  Unity Catalog alias can be repointed without touching the app, so the D1
  check reads the served model id off each response instead of trusting the
  config. Both the alias and the served id are pinned in
  `tests/test_d1_guards.py`.
- **One model still serves every role**: router, synthesizer, extractor.
  `DATABRICKS_REASONING_EFFORT` stays `low`.

## Prod Postgres is Databricks Lakebase; the Jul 28 "Supabase stays" call is reversed (recorded Aug 11 2026)

- **What changed.** The 2026-07-28 decision was "Databricks for inference only,
  Supabase stays the datastore". That call was reversed. Production Postgres is
  Databricks Lakebase, inside the company's own tenant.
- **Where it lives.** Host
  `ep-super-hat-d8wkrjd9.database.us-east-2.cloud.databricks.com`, region
  us-east-2, database `databricks_postgres`, app role `regwatch_app`. pgvector
  is in the same database, so rows, vectors and audit all sit in one Postgres.
- **How we know it is the live one.** The Alembic head in the live database is
  `0020_eval_run`, which matches the repo head, nothing pending. It holds 5,494
  chunk rows, which is exactly the `corpus_count` that `/health` reports.
- **The cutover date is not recorded here.** Only the end state was verified,
  on 2026-08-11. The older Supabase reasoning above is left as the record of
  what was decided at the time.
- **Open, unchanged by this move:** least-privilege database credentials and a
  rehearsed restore drill.

## D1 data residency is closed (recorded Aug 11 2026)

- **All three legs sit inside the company's Databricks tenant:** generation
  (`workspace.default.regwatch`), query and corpus embeddings
...
- **All three flags are on.** `REGWATCH_PROSE_SYNTHESIS` (v6 prose format),
  `REGWATCH_LIVE_DRAFT` (live token streaming of the provisional draft over
  SSE) and `REGWATCH_SELECTIVE_CITATION` (v7 policy) are set on Fly. The Aug 10
  entry above recorded v7 shipping dark. It is not dark any more.
- **"Cite or refuse" is retired as the headline rule.** It described v5 and v6.
  The rule now is: cite the facts, talk like a person.
- **Three kinds of sentence.** A SOURCE FACT says what FDA guidance requires,
  recommends, permits or prohibits, and must end with the passage number it
  came from, like `[1]` or `[1, 3]`, right before the final period. REASONING is
  our own reading, going past what the passages say; it carries no number and
  must open with one of four exact phrases ("The guidance does not state this
  directly; my reading is ...", "Reading the guidance together, ...", "My
  reading is ...", "Beyond the guidance, ..."), pinned byte for byte to
  `turn_gate.REASONING_FRAME_PREFIXES`. CONVERSATION is greetings, offers,
  transitions and questions back to the user: plain text, no numbers, no FDA
  facts.
- **INV-1 did not move.** An uncited source fact is still dropped by the gate,
  in code, not in the prompt. An obligation or a prohibition cannot hide inside
  a reasoning sentence: the source-assertion lexicon reclassifies it back to
  SOURCE FACT first.
- **No sentinel and no code word for "not found".** v6 had one and it broke 11
  of 11 refusals. In v7, when the passages do not answer the question, the model
  says so in ordinary words, names what it does have nearby, and offers a next
  step. The whole reply is plain prose with zero passage numbers. The verdict
  for it is `conversational_decline`.
- **What did not change.** `REFUSAL_SCORE_THRESHOLD` still defaults to 0.30 and
  still withholds weak passages before the synthesizer ever runs. Product scope
  is still resolved before retrieval, so citations cannot cross drugs (INV-9).
